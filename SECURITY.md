# Security Policy

## Supported Versions

| Version | Supported |
| --- | --- |
| v0.1.0 | Yes |
| < v0.1.0 | No |

Versions before `v0.1.0` are not supported and should not be deployed.

## Reporting a Vulnerability

Do not open a public issue for vulnerabilities.

Email: `sec.vyrox@proton.me`

Subject format:

```text
SECURITY: <brief description>
```

Response SLA:

- Acknowledgement within 48 hours
- Initial triage within 7 days
- Patch timeline communicated within 14 days

PGP key available at https://vyrox.dev/.well-known/pgp-key.txt.

## Scope

In scope:

- Authentication or authorization weaknesses
- Integrity issues in published security guidance
- Vulnerabilities in scripts or examples that affect deployments

Out of scope:

- UI preferences and style disputes
- Physical-access attack scenarios

## Disclosure Policy

Vyrox follows coordinated disclosure. Reporters are credited unless anonymity is requested.

No bounty program is active during alpha.

## Known Limitations

- Free-tier platform constraints may affect demonstration environments.
- Alpha docs can lag implementation temporarily.

These are operational constraints, not vulnerabilities.
